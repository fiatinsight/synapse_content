    ...
    def create
      ...
      respond_to do |format|
        if @navigation_group.save
          format.html { redirect_to main_app.send(FiatPublication.new_navigation_group_redirect_path, @navigation_group), notice: 'Group was created.' }
        else
    ...
    def destroy
      ...
      respond_to do |format|
        if params[:nested_parent_id]
          format.html { redirect_to main_app.send(FiatPublication.navigation_groups_path, params[:nested_parent_id]), notice: 'Group was successfully deleted.' }
        else
          format.html { redirect_to main_app.send(FiatPublication.navigation_groups_path), notice: 'Group was successfully deleted.' }
        end
      end
    end
