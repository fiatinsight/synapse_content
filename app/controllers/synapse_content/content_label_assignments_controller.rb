module SynapseContent
  class ContentLabelAssignmentsController < ActionController::Base
    ...
    def create
      unless SynapseContent::ContentLabelAssignment.find_by(content_label_id: params[:content_label_id], assignable_type: params[:assignable_type], assignable_id: params[:assignable_id])
        @content_label_assignment = SynapseContent::ContentLabelAssignment.create(content_label_id: params[:content_label_id], assignable_type: params[:assignable_type], assignable_id: params[:assignable_id])
      end

      respond_to do |format|
        if @content_label_assignment.save
          format.js { render :partial => 'synapse_content/content_label_assignments/create.js.erb', layout: false }
        end
      end
    end
    ...
    def destroy
      ...
      respond_to do |format|
        format.js { render :partial => 'synapse_content/content_label_assignments/destroy.js.erb', layout: false }
      end
    end
